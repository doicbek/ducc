/*
 * DUCC0 FFT DST MEX Interface
 * 
 * MATLAB MEX gateway for discrete sine transforms
 * 
 * Usage:
 *   out = ducc0_fft_dst_mex(in, type, axes, inorm, nthreads)
 * 
 * Inputs:
 *   in: Real input array (double or single)
 *   type: DST type (1, 2, 3, or 4) (required)
...
T computeNormFactor(int inorm, const vector<size_t> &shape, const vector<size_t> &axes, int type)
{
    ...
    // For DST, normalization uses factor 2 for all types except type 1
    if (type == 1) {
        // Type 1: factor 2*(N+1)
        N = 2 * (N + 1);
    } else {
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    try {
        // Check inputs
        if (nrhs < 2) {
            mexErrMsgIdAndTxt("DUCC0:FFT:DST:InputError", 
